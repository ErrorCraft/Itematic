...
public class BundleTestSuite {
    ...
    public void addingNormalItemToBundleAddsIt(GameTestHelper helper) {
        ...
        helper.succeedIf(() -> {
            Assert.isTrue(
                ...
                () -> "Expected left clicking on slot with Bundle to be successful"
            );
    ...
    public void addingShulkerBoxToBundleRejectsIt(GameTestHelper helper) {
        ...
        helper.succeedIf(() -> {
            Assert.isTrue(
                helper,
                bundleStack.overrideStackedOnOther(slot, ClickAction.PRIMARY, player),
                () -> "Expected left clicking on slot with Bundle to be successful"
            );
            Assert.itemStack(helper, slot.getItem())
                .isNotEmpty();
    ...
    public void addingBundleToBundleAddsItWithPenalty(GameTestHelper helper) {
        ...
        helper.succeedIf(() -> {
            ...
            Assert.areEqual(
                helper,
                Assert.isNotNull(helper, ItemHolderItemBehavior.occupancy(bundle), "possible occupancy")
                    .getOrThrow(),
