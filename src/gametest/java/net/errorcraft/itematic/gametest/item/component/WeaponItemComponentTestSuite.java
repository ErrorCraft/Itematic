...
public class WeaponItemComponentTestSuite {
    ...
    public void zombieAttackingWithIronSwordDealsCorrectDamage(TestContext context) {
        ...
        context.createTimedTaskRunner().expectMinDurationAndRun(1, () -> {
            ...
            Assert.areDoublesEqual(
                victim.getHealth(),
                MAX_HEALTH_VICTIM - zombie.getAttributeBaseValue(EntityAttributes.GENERIC_ATTACK_DAMAGE) - TestUtil.getItemComponent(stack, ItemComponentTypes.WEAPON).attackDamage().defaultDamage(),
                (value, expected) -> "Expected health to be " + expected + ", got " + value + " instead"
            );
        }).completeIfSuccessful();
    }
    ...
    public void piglinAttackingWithIronSwordDealsCorrectDamage(TestContext context) {
        ...
        context.createTimedTaskRunner().expectMinDurationAndRun(1, () -> {
            ...
            Assert.areDoublesEqual(
                victim.getHealth(),
                MAX_HEALTH_VICTIM - piglin.getAttributeBaseValue(EntityAttributes.GENERIC_ATTACK_DAMAGE) - TestUtil.getItemComponent(stack, ItemComponentTypes.WEAPON).attackDamage().defaultDamage(),
                (value, expected) -> "Expected health to be " + expected + ", got " + value + " instead"
            );
        }).completeIfSuccessful();
    }
