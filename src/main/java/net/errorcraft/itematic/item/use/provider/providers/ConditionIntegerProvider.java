...
public record ConditionIntegerProvider(IntegerProvider ticks, ItemPredicate condition) implements IntegerProvider {
    public static final MapCodec<ConditionIntegerProvider> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(
        IntegerProvider.CODEC.fieldOf("ticks").forGetter(ConditionIntegerProvider::ticks),
    ...
    public static final PacketCodec<RegistryByteBuf, ConditionIntegerProvider> PACKET_CODEC = PacketCodec.tuple(
        IntegerProvider.PACKET_CODEC, ConditionIntegerProvider::ticks,
