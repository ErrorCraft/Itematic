...
public class LeadTestSuite {
    ...
    public void usingLeadOnHorseLeashesHorse(GameTestHelper helper) {
        ...
        Horse horse = TestUtil.createEntityAt(helper, EntityTypes.HORSE, PLACED_ENTITY_POSITION, _ -> {});
        helper.succeedIf(() -> {
    ...
    public void usingLeadOnBoatLeashesBoat(GameTestHelper helper) {
        ...
        player.setItemInHand(InteractionHand.MAIN_HAND, lead);
        level.addFreshEntity(player);
        Boat boat = TestUtil.createEntityAt(helper, EntityTypes.OAK_BOAT, PLACED_ENTITY_POSITION, _ -> {});
        helper.succeedIf(() -> {
