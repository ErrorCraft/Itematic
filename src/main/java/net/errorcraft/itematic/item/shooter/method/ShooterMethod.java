...
public interface ShooterMethod {
    Codec<ShooterMethod> CODEC = ItematicRegistries.SHOOTER_METHOD_TYPE.byNameCodec().dispatch(ShooterMethod::type, ShooterMethodType::codec);

    ShooterMethodType<?> type();
    void addComponents(DataComponentMap.Builder builder);
    boolean tryShoot(ShooterItemComponent component, ItemStack stack, Level world, LivingEntity user, InteractionHand hand);
    void hold(ShooterItemComponent shooter, ItemStack stack, Level world, LivingEntity user, int usedTicks);
    boolean stop(ShooterItemComponent shooter, ItemStack stack, Level world, LivingEntity user, int usedTicks);
